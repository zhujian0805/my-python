...
def hashhosts(pl):
    ...
    pl = pl.lower()
    try:
        matchobj = re.match(r'(.*=?)r.*(\d{1,})e(\d{1,})b(\d{1,})', pl,
                            re.M | re.I)
    except:
        return

    parent = matchobj.group(1) + "-rack0" + matchobj.group(
        2) + "-enclosure0" + matchobj.group(3)
...
def locinfo(locs, searchvar, type="plocation"):
    ...
    if type == "plocation":
        ...
        #pprint(hashhost)
        for dn, attr in locs:
            if attr.has_key('parent') and hashhost.has_key(
                    attr['parent'][0]) and attr['section'][0] in hashhost[
                        attr['parent'][0]]['section']:
                return attr
    if type == "hostname":
        for dn, attr in locs:
    ...
    if type == "ipaddr":
        for dn, attr in locs:
            if attr.has_key('interface') and strinlist(searchvar,
                                                       attr['interface']):
                return attr
    return
...
def main():
    ...
    locations = l.search_s("ou=locations,dc=sample,dc=net",
                           ldap.SCOPE_ONELEVEL)

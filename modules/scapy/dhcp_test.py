...
class DHCP_test(object):
  ...
  def build_disc(self):
    ...
    self.dhcp_disc = Ether(src = self.hw, dst = "ff:ff:ff:ff:ff:ff", type=0x0800)/IP(src = "0.0.0.0", dst = "255.255.255.255")/UDP(sport = 68, dport = 67)/BOOTP(chaddr = self.hw)/DHCP(options = [("message-type", "discover"), "end"])
  ...
  def run(self):
    ...
    for offer in ans_for_disc:
      ...
      dhcp_request[DHCP].options.append(("param_req_list", b'x01x1c2x03x0fx06x77x0cx2cx2fx1ax79x2a'))

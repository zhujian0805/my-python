...
rabbit_host_opt = cfg.StrOpt(
    'name', default='localhost', help='IP/hostname to listen on.')

conf.register_opt(rabbit_host_opt)

print conf.name

rabbit_group = cfg.OptGroup(name='testing', title='testing')
rabbit_host_opt = cfg.StrOpt(
    'host', default='localhost', help='IP/hostname to listen on.')
...
vxlan_group = cfg.OptGroup(name='vxlan', title='vxlan')
vxlan_opts = [
    cfg.BoolOpt(
        'enable_vxlan',
        default=True,
        help=("Enable VXLAN on the agent. Can be enabled when "
              ...
              "mechanism driver")),
    cfg.IntOpt('ttl', help=("TTL for vxlan interface protocol packets.")),
    cfg.IntOpt('tos', help=("TOS for vxlan interface protocol packets.")),
    cfg.StrOpt(
        'vxlan_group',

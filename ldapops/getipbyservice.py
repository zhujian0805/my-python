...
def main(server, password, service):
	...
	services = l.search_s("ou=services,dc=sample,dc=net",ldap.SCOPE_SUBTREE, "(service=%s*)" % service)

        ips = []
        for dn, attrs in services:
            print attrs['address'][0].strip(),

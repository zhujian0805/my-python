    ...
    try:
        ...
        try:
            for bid in BUILD_ECONOMY:
                ...
                print(result[1])
            print("Successfully reloaded the build info for the 'in progress' builds.")
            logging.info("Successfully reloaded the build info for the 'in progress' builds.")
        except:
            print("Could not reload build info for build with id %s" % bid)
            logging.info("Could not reload build info for build with id %s" % bid)
    except:
...
class Builder(rpyc.Service):
    ...
    def exposed_save_config(self, type, xml_string):
        ...
        XML(xml_string)
        file_path = os.path.join(Config.get("config"),
                                 Config.get(type))
    ...
    def exposed_reset(self, p_id, dry_run):
        ...
        return (SUCCESS, "Changed build state to: %d." % BuildStates.BUILD_STATE_WAITING)
...
%s""" % (str(BUILD_ECONOMY), ConfigXML.apps.pprint(),
    ...
    def exposed_get_apps_info(self, specified_region=None, name=None):
        ...
        if specified_region == None:
            self.logger.error("Region must be set. Look at the '-R' flag in the help.")
            return (ERROR, "ERROR: Region must be set. Look at the '-R' flag in the help.")
        ...
        # return FAILURE
        if name is not None:
            self.logger.error("Application with name %s could not be found." % name)
            return (ERROR, "ERROR: Application with name %s could not be found." % name)
        return (SUCCESS, apps)
    ...
    def exposed_new_build(self, p_id, p_name, specified_region, dry_run):
        ...
        self.logger.info("Setting up build id %s for application %s." % (p_id, p_name))
        for app in ConfigXML.apps.get_apps(specified_region):
            if app["name"] == p_name:
                ...
                logr_name = "%s.%s**%s**%s**%s" % (self.logger_name, app["region"], p_name, p_id,
                                                   time.strftime(Config.get("log_timestamp_fmt", specified_region)))
                logr = logging.getLogger(logr_name)
                handler = logging.FileHandler("%s/%s.log" % (Config.get("logs", specified_region), logr_name))
                handler.setFormatter(logging.Formatter(Config.get("log_string_fmt", specified_region)))
                logr.addHandler(handler)
                BUILD_ECONOMY[p_id] = (Application(copy.deepcopy(app), logr),
                                       {"status" : BuildStates.BUILD_STATE_WAITING,
                                        "logger" : logr,
                                        "logger_name" : logr_name,
                                        "timestamp" : time.strftime(Config.get("log_timestamp_fmt", specified_region))})
                self.logger.debug("Successful setup application %s with build id %s." % (app["name"], p_id))
                return (SUCCESS, p_id)
        self.logger.error("No application found with name %s." % p_name)
        return (ERROR, "ERROR: No application found with that name %s." % p_name)
    ...
    def exposed_app_update(self, p_id, revision, checkout, dry_run):
        ...
        if BUILD_ECONOMY[p_id][1]["status"] != BuildStates.BUILD_STATE_WAITING:
            if BUILD_ECONOMY[p_id][1]["status"] == BuildStates.BUILD_STATE_ERRORED:
                self.logger.error("A previous command to this application (%s) errored out." % BUILD_ECONOMY[p_id][0].name)
                return (ERROR, "ERROR: A previous command to this application errored out.")
            self.logger.error("There is an ongoing operation on application %s." % BUILD_ECONOMY[p_id][0].name)
            return (ERROR, "ERROR: There is an ongoing operation on this application.")
        ...
        print "application name: %s, application svn path: %s, apppath: %s" % (appname, appsvn, apppath)
        BUILD_ECONOMY[p_id][1]["status"] = BuildStates.BUILD_STATE_SVN_UPDATE
        if revision == "HEAD":
            rev, msg, changes = appsvn.update(checkout=checkout)
        else:
            rev, msg, changes = appsvn.update(rev=int(revision), checkout=checkout)
        ...
        if rev == -1 and msg == -2 and changes == []:
            ...
            BUILD_ECONOMY[p_id][1]["status"] = BuildStates.BUILD_STATE_ERRORED
            self.logger.error("Could not update because I could not connect to the repository.")
            return (ERROR, "ERROR: Could not update because I could not connect to the repository.")

        tmpstr = "Project %s in svn location %s was updated successfully." % (appname, apppath)
        self.logger.info(tmpstr)
        applogger.info(tmpstr)
    ...
    def exposed_app_status(self, p_id, dry_run):
        ...
        return (SUCCESS, "projectName=%s,buildId=%s,buildState=%s" % (
                BUILD_ECONOMY[p_id][0].name, p_id, BUILD_ECONOMY[p_id][1]["status"]))
    ...
    def exposed_app_expire(self, p_id, dry_run):
        ...
        if p_id == "all":
            for id_ in BUILD_ECONOMY:
                BUILD_ECONOMY[id_][1]["logger"].info("This build has just expired!")
            BUILD_ECONOMY.clear()
    ...
    def exposed_app_build(self, p_id, clean, bflags, dry_run):
        ...
        if BUILD_ECONOMY[p_id][0].ant == None:
            BUILD_ECONOMY[p_id][1]["status"] = BuildStates.BUILD_STATE_ERRORED
            self.logger.error("This application has no dynamic content to be built.")
            return (ERROR, "ERROR: This application has no dynamic content to be built.")

        isjava6 = False
        print(BUILD_ECONOMY[p_id][0].flags)
        if BUILD_ECONOMY[p_id][0].flags and (BUILD_ECONOMY[p_id][0].flags.get("java6", "no") == "yes"):
            isjava6 = True
        ...
        if clean:
            if BUILD_ECONOMY[p_id][0].ant is None:
                # BUILD_ECONOMY[p_id][1]["status"] = BuildStates.BUILD_STATE_WAITING
                BUILD_ECONOMY[p_id][1]["status"] = BuildStates.BUILD_STATE_ERRORED
                self.logger.error("No build type for application %s." % appname)
                return (ERROR, "ERROR: No build type on this application.")

            isold = False
            if BUILD_ECONOMY[p_id][0].flags and (BUILD_ECONOMY[p_id][0].flags.get("old", "no") == "yes"):
                isold = True
            applogger.info("Performing an ant clean.")
            res_clean = BUILD_ECONOMY[p_id][0].ant.clean(bflags, isold, isjava6)
        ...
        if BUILD_ECONOMY[p_id][0].flags and (BUILD_ECONOMY[p_id][0].flags.get("old", "no") == "yes"):
            applogger.info("This is a build that does not follow Simple Template Documentation.")
        ...
        if res_clean:
            return (res[0], (res_clean[1][0] + res[1][0], res_clean[1][1] + res[1][1]))
        return (res[0], res[1])
    ...
    def _try_extract(self, svn_path, target, name, applogger, lastdir):
        ...
        if res == 0:
            ...
            applogger.error("Failed to execute 'tar xzf %s/dist/*-%s.tar.gz -C %s/%s/'." % (svn_path, name, target, lastdir))
            logging.error("Failed to execute 'tar xzf %s/dist/*-%s.tar.gz -C %s/%s/'." % (svn_path, name, target, lastdir))
            applogger.error("Will try to untar from target directory %s." % target)
            logging.error("Will try to untar from target directory %s." % target)
            applogger.info("Extracting tarball %s/dist/*-%s.tar.gz into %s/%s/" % (target, name, target, lastdir))
            res = os.system("tar xzf %s/*-%s.tar.gz -C %s/%s/" % (target, name, target, lastdir))
            if res != 0:
                applogger.error("Failed to execute 'tar xzf %s/*-%s.tar.gz -C %s/%s/'." % (target, name, target, lastdir))
                logging.error("Failed to execute 'tar xzf %s/*-%s.tar.gz -C %s/%s/'." % (target, name, target, lastdir))
            else:
                applogger.info("Successfully extracted tarball."
                               "Removed tarball.")
    ...
    def _extract_tarball(self, p_id, type_of_content, dry_run):
        ...
        target = "%s/%s-%s" % (
            ...
            Config.get("target", BUILD_ECONOMY[p_id][0].region), BUILD_ECONOMY[p_id][0].region)
        ...
        if type_of_content == "static":
            ...
            res = self._try_extract(BUILD_ECONOMY[p_id][0].svn.path,
                                    target, "www", applogger, "www")
            if res != 0:
                applogger.info("Trying to extract using type of content: %s" % type_of_content)
                res = self._try_extract(BUILD_ECONOMY[p_id][0].svn.path,
                                        target, type_of_content,
                                        applogger, "www")

        elif type_of_content == "dynamic":
            applogger.debug("Creating folder %s/webapp/" % target)
            functions.exec_oe("mkdir -p %s/webapp/" % target)
            res = self._try_extract(BUILD_ECONOMY[p_id][0].svn.path, target, "webapp", applogger, "webapp")
            if res != 0:
                applogger.info("Trying to extract using type of content: %s" % type_of_content)
                res = self._try_extract(BUILD_ECONOMY[p_id][0].svn.path, target, type_of_content, applogger, "webapp")

        if res == 0:
            applogger.info("Successfully extracted %s tarball for application %s." % (type_of_content, BUILD_ECONOMY[p_id][0].name))
            logging.info("Successfully extracted %s tarball for application %s." % (type_of_content, BUILD_ECONOMY[p_id][0].name))
            return (SUCCESS, "%s tarball for %s extracted." % (type_of_content.capitalize(), BUILD_ECONOMY[p_id][0].name))
        else:
            applogger.error("Failed to extract %s tarball for %s." % (type_of_content.capitalize(), BUILD_ECONOMY[p_id][0].name))
            return (ERROR, "Failed to extract %s tarball for %s." % (type_of_content.capitalize(), BUILD_ECONOMY[p_id][0].name))
    ...
    def _move_content_into_target(self, p_id, type_of_content, dry_run):
        ...
        if flags.get("loc%s" % type_of_content.capitalize()) is not None:
            src_dir = "%s%s" % (BUILD_ECONOMY[p_id][0].svn.path, flags.get("loc%s" % type_of_content.capitalize()))
        else:
            if not os.path.isdir(src_dir):
                if type_of_content == "static":
                    src_dir = "%s%s" % (BUILD_ECONOMY[p_id][0].svn.path, "/dist/www/")
                elif type_of_content == "dynamic":
                    src_dir = "%s%s" % (BUILD_ECONOMY[p_id][0].svn.path, "/dist/webapp/")

        applogger.info("Source directory for this application will be: %s" % src_dir)

        res = -1
        # set up the common rsync command line we want to use
        rsync_cmdline = 'rsync %s --delete --exclude "%s*" %s %s/*' % (Config.get("rsync_opts"),
                                                                       Config.get("target")[1:],
                                                                       Config.get("rsync_drone_excludes"),
                                                                       src_dir)
        ...
        if type_of_content == "static":
            ...
            if res == 0:
                ...
                applogger.debug("Sync was successful, removing folder %s/www/%s*" % (target, Config.get("target", BUILD_ECONOMY[p_id][0].region).strip("/").split("/")[0]))
                res = os.system("rm -rf %s/www/%s*" % (target, Config.get("target", BUILD_ECONOMY[p_id][0].region).strip("/").split("/")[0]))
        elif type_of_content == "dynamic":
            applogger.debug("Creating folder %s/webapp/" % target)
            functions.exec_oe("mkdir -p %s/webapp/" % target)
            # rsyncs content into target directory ignoring the target directory
            # else applications like launcher (EU old) would not work
            applogger.debug('Executing: %s %s/webapp/' % (rsync_cmdline, target))
            res = os.system("%s %s/webapp/" % (rsync_cmdline, target))
            if res == 0:
                # try to delete directory TARGET if it was copied
                applogger.debug("Sync was successful, removing folder %s/webapp/%s" % (target, Config.get("target", BUILD_ECONOMY[p_id][0].region).strip("/").split("/")[0]))
                res = os.system("rm -rf %s/webapp/%s*" % (target, Config.get("target", BUILD_ECONOMY[p_id][0].region).strip("/").split("/")[0]))

        if res == 0:
            applogger.info("Successfully copied the %s content to the correct location for application %s." % (
                    type_of_content, BUILD_ECONOMY[p_id][0].name))
            logging.info("Successfully copied the %s content to the correct location for application %s." % (
                    type_of_content, BUILD_ECONOMY[p_id][0].name))
            return (SUCCESS, "%s content for %s copied to the correct location.\n" % (
                    type_of_content.capitalize(), BUILD_ECONOMY[p_id][0].name))
        applogger.info("Failed to copy the %s content to the correct location for application %s." % (
                type_of_content, BUILD_ECONOMY[p_id][0].name))
        logging.info("Failed to copy the %s content to the correct location for application %s." % (
                type_of_content, BUILD_ECONOMY[p_id][0].name))
    ...
    def _rsync(self, p_id, type_of_content, rsync_opts, old, dry_run):
        ...
        if dry_run:
            ...
            if type_of_content == "static":
                applogger.info('Executing: rsync %s %s %s %s/dist/%s/www/ %s:DESTINATION_ON_NFS_FOLDER' % (
                    Config.get("rsync_opts"), rsync_opts, Config.get("rsync_nfs_excludes"),
                    BUILD_ECONOMY[p_id][0].svn.path, BUILD_ECONOMY[p_id][0].ant.binfo.get("build.type", ""),
                    Config.get("nfs_ip", BUILD_ECONOMY[p_id][0].region)))

            elif type_of_content == "dynamic":
                applogger.info('Executing: rsync %s %s %s %s/dist/%s/webapp/ %s:DESTINATION_ON_NFS_FOLDER' % (
                    Config.get("rsync_opts"), rsync_opts, Config.get("rsync_nfs_excludes"),
                    BUILD_ECONOMY[p_id][0].svn.path,
                    BUILD_ECONOMY[p_id][0].ant.binfo.get("build.type", ""),
                    Config.get("nfs_ip", BUILD_ECONOMY[p_id][0].region)))
            return (SUCCESS, "Deployed %s %s content to the nfs." % (BUILD_ECONOMY[p_id][0].name, type_of_content))
        ...
        rcode, output, error = functions.exec_oe("ssh %s %s/apps.py --type %s %s" % (
                Config.get("nfs_ip", BUILD_ECONOMY[p_id][0].region),
                Config.get("nfs_utils", BUILD_ECONOMY[p_id][0].region),
                type_of_content, BUILD_ECONOMY[p_id][0].name))
        ...
        if len(output) > 0:
            ...
            applogger.info("FAILURE: Tried to rsync %s content with a top dir in the NFS." % type_of_content)
            logging.info("FAILURE: Tried to rsync %s content with a top dir in the NFS." % type_of_content)
            return (ERROR, "Tried to rsync %s content with a top dir in the NFS ." % type_of_content)

        res = -1
        if type_of_content == "static":
            if old:
                applogger.info("Syncing application from %s/%s-%s/www/ to %s:%s." % (
                        BUILD_ECONOMY[p_id][0].svn.path, Config.get("target", BUILD_ECONOMY[p_id][0].region),
                        BUILD_ECONOMY[p_id][0].region, Config.get("nfs_ip", BUILD_ECONOMY[p_id][0].region), nfs_folder))
                res = os.system(r'rsync %s %s %s %s/%s-%s/www/ %s:%s' % (
                        Config.get("rsync_opts"), rsync_opts, Config.get("rsync_nfs_excludes"),
                        BUILD_ECONOMY[p_id][0].svn.path,
                        Config.get("target", BUILD_ECONOMY[p_id][0].region), BUILD_ECONOMY[p_id][0].region,
                        Config.get("nfs_ip", BUILD_ECONOMY[p_id][0].region), nfs_folder))
            else:
                applogger.info('Executing: rsync %s %s %s %s/dist/%s/www/ %s:%s' % (
                        Config.get("rsync_opts"), rsync_opts, Config.get("rsync_nfs_excludes"),
                        BUILD_ECONOMY[p_id][0].svn.path, BUILD_ECONOMY[p_id][0].ant.binfo.get("build.type", ""),
                        Config.get("nfs_ip", BUILD_ECONOMY[p_id][0].region), nfs_folder))

                res = os.system(r'rsync %s %s %s %s/dist/%s/www/ %s:%s' % (
                        Config.get("rsync_opts"), rsync_opts, Config.get("rsync_nfs_excludes"),
                        BUILD_ECONOMY[p_id][0].svn.path, BUILD_ECONOMY[p_id][0].ant.binfo.get("build.type", ""),
                        Config.get("nfs_ip", BUILD_ECONOMY[p_id][0].region), nfs_folder))
        ...
        if type_of_content == "dynamic":
            if old:
                applogger.info("Syncing application from %s/%s-%s/webapp/ to %s:%s." % (
                        BUILD_ECONOMY[p_id][0].svn.path, Config.get("target", BUILD_ECONOMY[p_id][0].region),
                        BUILD_ECONOMY[p_id][0].region, Config.get("nfs_ip", BUILD_ECONOMY[p_id][0].region), nfs_folder))
                res = os.system(r'rsync %s %s %s %s/%s-%s/webapp/ %s:%s' % (
                        Config.get("rsync_opts"), rsync_opts, Config.get("rsync_nfs_excludes"),
                        BUILD_ECONOMY[p_id][0].svn.path,
                        Config.get("target", BUILD_ECONOMY[p_id][0].region), BUILD_ECONOMY[p_id][0].region,
                        Config.get("nfs_ip", BUILD_ECONOMY[p_id][0].region), nfs_folder))
            else:
                applogger.info('Executing: rsync %s %s %s %s/dist/%s/webapp/ %s:%s' % (
                        Config.get("rsync_opts"), rsync_opts, Config.get("rsync_nfs_excludes"),
                        BUILD_ECONOMY[p_id][0].svn.path,
                        BUILD_ECONOMY[p_id][0].ant.binfo.get("build.type", ""),
                        Config.get("nfs_ip", BUILD_ECONOMY[p_id][0].region), nfs_folder))

                res = os.system(r'rsync %s %s %s %s/dist/%s/webapp/ %s:%s' % (
                        Config.get("rsync_opts"), rsync_opts, Config.get("rsync_nfs_excludes"),
                        BUILD_ECONOMY[p_id][0].svn.path,
                        BUILD_ECONOMY[p_id][0].ant.binfo.get("build.type", ""),
                        Config.get("nfs_ip", BUILD_ECONOMY[p_id][0].region), nfs_folder))
        if res == 0:
            ...
            res_own = os.system(r'ssh %s %s -R %s.%s %s' % (Config.get("nfs_ip", BUILD_ECONOMY[p_id][0].region),
                                                            ...
                                                            nfs_folder))
            if res_own == 0:
                applogger.info("Successfully changed ownership (%s.%s) on the nfs for folder %s ." % (
                        Config.get("nfs_user", BUILD_ECONOMY[p_id][0].region),
                        Config.get("nfs_group", BUILD_ECONOMY[p_id][0].region),
                        nfs_folder))
            else:
                # it's ok to not return error here
                applogger.info("Could not change ownership (%s.%s) on the nfs for folder %s ." % (
                        Config.get("nfs_user", BUILD_ECONOMY[p_id][0].region),
                        Config.get("nfs_group", BUILD_ECONOMY[p_id][0].region),
                        nfs_folder))

            applogger.info("Deployed %s %s content to the nfs." % (BUILD_ECONOMY[p_id][0].name, type_of_content))
            return (SUCCESS, "Deployed %s %s content to the nfs." % (BUILD_ECONOMY[p_id][0].name, type_of_content))
        applogger.error("Could not deploy %s %s content to the nfs." % (BUILD_ECONOMY[p_id][0].name, type_of_content))
        return (ERROR, "Could not deploy %s %s content to the nfs." % (BUILD_ECONOMY[p_id][0].name, type_of_content))

    def _check_content(self, flags, type_content):
        if flags:
            if flags.get("static", "yes") == "yes" and type_content == "static":
                return type_content
            if flags.get("dynamic", "yes") == "yes" and type_content == "dynamic":
                return type_content
        else:
            if type_content:
                return type_content
    ...
    def _app_deploy_old(self, p_id, type_content, rsync_delete, nonfs, dry_run):
        """
        ...
        if nonfs:
            ...
            res = self._rsync_deploy(p_id, type_content, flags, rsync_delete, True, dry_run)
    ...
    def _rsync_deploy(self, p_id, type_content, flags, rsync_delete, old=False, dry_run=False):
        ...
        if self._check_content(flags, type_content) == None:
            ...
            if flags is None:
                ...
                msg += "\n" + res_static[1]
                res_dynamic = self._rsync(p_id, "dynamic", rsync_opts, old, dry_run)
    ...
    def exposed_app_deploy(self, p_id, flags_, type_content, rsync_delete, nonfs, region, dry_run):
        """
        ...
        if flags:
            ...
            if flags.get("old", "no") == "yes":
                result = self._app_deploy_old(p_id, type_content, rsync_delete, nonfs, dry_run)
                BUILD_ECONOMY[p_id][1]["status"] = BuildStates.BUILD_STATE_WAITING
        ...
        if nonfs:
            applogger.info("nonfs flag was passed and this is an application that follows the Simple Template Documentation.")
            applogger.info("You will not see any content on the nfs at this point.")
            BUILD_ECONOMY[p_id][1]["status"] = BuildStates.BUILD_STATE_WAITING
            if tmp_region is not None:
                BUILD_ECONOMY[p_id][0].region = tmp_region
            return (SUCCESS, "")

        res = self._rsync_deploy(p_id, type_content, flags, rsync_delete, dry_run=dry_run)
        BUILD_ECONOMY[p_id][1]["status"] = BuildStates.BUILD_STATE_WAITING
        if tmp_region is not None:
            BUILD_ECONOMY[p_id][0].region = tmp_region
        return (res[0], res[1])
    ...
    def _run_deploy(self, p_id, cmd, ip=None, appname=None, type_content=None):
        if ip != None:
            return os.system(cmd % (Config.get("nfs_ip", BUILD_ECONOMY[p_id][0].region),
                                    Config.get("nfs_utils", BUILD_ECONOMY[p_id][0].region),
                                    ip))
        ...
        if (appname == None) and (type_content == None):
            ...
            return -1
        self.logger.info(cmd % (Config.get("nfs_ip", BUILD_ECONOMY[p_id][0].region),
                                Config.get("nfs_utils", BUILD_ECONOMY[p_id][0].region),
    ...
    def exposed_app_restart(self, p_id, type_content, ip, region, dry_run):
        ...
        if ip != "":
            ...
            if res == 0:
                return (SUCCESS, "Successfully restarted the tomcat on ip %s." % ip)
        ...
        res = -1
        if type_content:
            res = self._run_deploy(p_id, DeployController.RESTART, appname=appname, type_content=type_content)
        else:
            msg = ""
            if BUILD_ECONOMY[p_id][0].flags.get("static", "yes") == "yes":
                res = self._run_deploy(p_id, DeployController.RESTART, appname=appname, type_content="static")
                if res == 0:
                    msg = "%s%s" % (msg, "Successfully touched deploy file for static.\n\n")
                else:
                    msg = "%s%s" % (msg, "Could not touch deploy file for static.\n\n")

            if BUILD_ECONOMY[p_id][0].flags.get("dynamic", "yes") == "yes":
                res = self._run_deploy(p_id, DeployController.RESTART, appname=appname, type_content="dynamic")
                if res == 0:
                    msg = "%s%s" % (msg, "Successfully touched deploy file for dynamic and restart the tomcats for application %s with id %s.\n\n" % (
                        appname, p_id))
                else:
                    msg = "%s%s" % (msg, "Could not touch deploy file for dynamic and restart the tomcats.\n\n")
        ...
        if res == 0:
            applogger.info("Successfully restarted the tomcats for application %s with id %s." % (appname, p_id))
            return (SUCCESS, "Successfully restarted the tomcats for application %s with id %s." % (appname, p_id))
        applogger.info("Could not restart the tomcats for application %s with id %s." % (appname, p_id))
        return (ERROR, "Could not restart the tomcats for application %s with id %s." % (appname, p_id))
    ...
    def exposed_app_sync(self, p_id, type_content, region, dry_run):
        ...
        res = -1
        if type_content:
            res = self._run_deploy(p_id, DeployController.SYNC, appname=appname, type_content=type_content)
        else:
            msg = ""
            if BUILD_ECONOMY[p_id][0].flags.get("static", "yes") == "yes":
                res = self._run_deploy(p_id, DeployController.SYNC, appname=appname, type_content="static")
                if res == 0:
                    msg = "%s%s" % (msg, "Successfully touched deploy file for static.\n\n")
                else:
                    msg = "%s%s" % (msg, "Could not touch deploy file for static.\n\n")
            ...
            if BUILD_ECONOMY[p_id][0].flags.get("dynamic", "yes") == "yes":
                res  = self._run_deploy(p_id, DeployController.SYNC, appname=appname, type_content="dynamic")
                if res == 0:
                    msg = "%s%s" % (msg, "Successfully touched deploy file for dynamic.\n\n")
                else:
                    msg = "%s%s" % (msg, "Could not touch deploy file for dynamic.\n\n")
...
if __name__ == '__main__':
    logging.basicConfig(filename="%s/server-%s" % (Config.get("server_logs"),
                                                   time.strftime(Config.get("log_timestamp_fmt"))),
                        level=logging.DEBUG, format=Config.get("log_string_fmt"),
